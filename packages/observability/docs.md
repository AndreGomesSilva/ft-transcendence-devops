...
**Symptoms**: Console logs show connection errors
**Solutions**:
- Verify `LOGSTASH_HOST` and `LOGSTASH_PORT`
- Check network connectivity
- Ensure Logstash is configured for TCP input

#### 2. Metrics Not Appearing
**Symptoms**: `/metrics` endpoint returns empty or missing metrics
**Solutions**:
- Verify `enableMetrics: true` in configuration
- Check if metrics path is correct
- Ensure requests are being made to generate HTTP metrics

#### 3. High Memory Usage
**Symptoms**: Application memory usage grows over time
**Solutions**:
- Monitor metric cardinality (too many unique label combinations)
- Implement metric cleanup for dynamic labels
- Consider metric sampling for high-traffic services
...
#### `setupObservability(fastify, config)`
Main setup function that configures all observability features.

**Parameters:**
- `fastify: FastifyInstance` - Fastify application instance
- `config: string | ObservabilityConfig` - Service name or configuration object

**Returns:**
- `ObservabilitySetup` - Object containing logger and metrics registry

#### `setupLogging(serviceName, logLevel?)`
Sets up structured logging with Pino and optional Logstash integration.

#### `setupMetrics(fastify, serviceName, metricsPath?)`
Configures Prometheus metrics collection and HTTP endpoint.

#### `setupHealthCheck(fastify, serviceName, healthPath?)`
Adds health check endpoint to Fastify application.
...
```typescript
const { metricsRegistry } = setupObservability(fastify, 'my-service');

// Create custom metrics
const customGauge = new metricsRegistry.register.Gauge({
...
customGauge.set({ type: 'orders', region: 'us' }, 42);
```

### Custom Health Checks

```typescript
// Extend health endpoint with custom checks
...
This documentation provides comprehensive technical details for understanding, implementing, and extending the observability library in your services.
